...
void GameState::generatePawnMoveList(std::vector<BitMove>& moves, const BitBoard pawns, const BitBoard emptySquares, const BitBoard enemyPieces, char color) {
    ...
    BitBoard demoLeft(NotHFile);
        ...
        //SWITCH TO == WHITE IF PAWNS NOT WORKING
    ...
    int shiftForward = (color == BLACK) ? 8 : -8;
    int doubleShift = (color == BLACK) ? 16 : -16;
    int captureLeftShift = (color == BLACK) ? 7 : -9;
    int captureRightShift = (color == BLACK) ? 9 : -7;
